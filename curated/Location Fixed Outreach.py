...
output_folder_name = 'Location_Fixed_Outreach'
...
df_dimension = df_dimension.filter(col('name')==lit('Location Fixed/Outreach'))
...
df_Location_Fixed_Outreach= df_dimension.join(df_categoryOptions,df_dimension.items_id==df_categoryOptions.id,"left").select(df_categoryOptions['name'],'categoryOptionCombos_id')

# COMMAND ----------

display(df_Location_Fixed_Outreach)

# COMMAND ----------

df_Location_Fixed_Outreach.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
