...
df_dimension = df_dimension.filter(col('name')==lit('PMTCT'))
...
df_PMTCT = df_dimension.join(df_dataElementGroups,df_dimension.items_id==df_dataElementGroups.id,"left").select(df_dataElementGroups['name'],'dataElements_id').distinct()
...
df_PMTCT.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
