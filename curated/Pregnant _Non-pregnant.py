...
output_folder_name = 'Pregnant_Non-pregnant'
...
df_dimension = df_dimension.filter(col('name')==lit('Pregnant/Non-pregnant'))
...
df_Pregnant_Non_pregnant= df_dimension.join(df_categoryOptions,df_dimension.items_id==df_categoryOptions.id,"left").select(df_categoryOptions['name'],'categoryOptionCombos_id')

# COMMAND ----------

display(df_Pregnant_Non_pregnant)

# COMMAND ----------

df_Pregnant_Non_pregnant.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
