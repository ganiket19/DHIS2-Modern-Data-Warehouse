...
output_folder_name = 'Commodities'
...
df_dimension = df_dimension.filter(col('name')==lit('Commodities'))
...
df_commodities = df_dimension.join(df_categoryOptions,df_dimension.items_id==df_categoryOptions.id,"left").select(df_categoryOptions['name'],'categoryOptionCombos_id')
...
df_commodities.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
