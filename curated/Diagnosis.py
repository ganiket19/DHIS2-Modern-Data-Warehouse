...
df_dimension = df_dimension.filter(col('name')==lit('Diagnosis'))
...
display(df_dataElementGroups)

# COMMAND ----------

df_diagnosis = df_dimension.join(df_dataElementGroups,df_dimension.items_id==df_dataElementGroups.id,"left").select(df_dataElementGroups['name'],'dataElements_id').distinct()
...
df_diagnosis.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
