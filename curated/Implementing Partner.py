...
output_folder_name = 'Implementing_Partner'
...
df_dimension = df_dimension.filter(col('name')==lit('Implementing Partner'))
...
df_Implementing_Partner= df_dimension.join(df_categoryOptions,df_dimension.items_id==df_categoryOptions.id,"left").select(df_categoryOptions['name'],'categoryOptionCombos_id').withColumnRenamed('categoryOptionCombos_id','attributeOptionCombo_id')

# COMMAND ----------

display(df_Implementing_Partner)

# COMMAND ----------

df_Implementing_Partner.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
