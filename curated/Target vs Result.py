...
output_folder_name = 'TargetvsResult'
...
df_dimension = df_dimension.filter(col('name')==lit('Target vs Result'))
...
df_TargetvsResult= df_dimension.join(df_categoryOptions,df_dimension.items_id==df_categoryOptions.id,"left").select(df_categoryOptions['name'],'categoryOptionCombos_id')

# COMMAND ----------

display(df_TargetvsResult)

# COMMAND ----------

df_TargetvsResult.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
