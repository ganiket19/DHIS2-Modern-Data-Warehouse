...
output_folder_name = 'EPI_nutrition_age'
...
df_dimension = df_dimension.filter(col('name')==lit('EPI/nutrition age'))
...
df_EPI_nutrition_age = df_dimension.join(df_categoryOptions,df_dimension.items_id==df_categoryOptions.id,"left").select(df_categoryOptions['name'],'categoryOptionCombos_id')

# COMMAND ----------

display(df_EPI_nutrition_age)

# COMMAND ----------

df_EPI_nutrition_age.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
