...
df_dimension = df_dimension.filter(col('name')==lit('Project'))
...
df_Project= df_dimension.join(df_categoryOptions,df_dimension.items_id==df_categoryOptions.id,"left").select(df_categoryOptions['name'],'categoryOptionCombos_id').withColumnRenamed('categoryOptionCombos_id','attributeOptionCombo_id')
...
df_Project.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
