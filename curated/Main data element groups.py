...
output_folder_name = 'Main_data_element_groups'
...
df_dimension = df_dimension.filter(col('name')==lit('Main data element groups'))
...
df_Main_data_element_groups = df_dimension.join(df_dataElementGroups,df_dimension.items_id==df_dataElementGroups.id,"left").select(df_dataElementGroups['name'],'dataElements_id').distinct()

# COMMAND ----------

display(df_Main_data_element_groups)

# COMMAND ----------

df_Main_data_element_groups.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
