...
output_folder_name = 'Morbidity_Age'
...
df_dimension = df_dimension.filter(col('name')==lit('Morbidity Age'))
...
df_Morbidity_Age= df_dimension.join(df_categoryOptions,df_dimension.items_id==df_categoryOptions.id,"left").select(df_categoryOptions['name'],'categoryOptionCombos_id')

# COMMAND ----------

display(df_Morbidity_Age)

# COMMAND ----------

df_Morbidity_Age.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
