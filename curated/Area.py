...
df_dimension = df_dimension.filter(col('name')==lit('Area'))
...
df_Area = df_dimension.join(df_organisationUnitGroups,df_dimension.items_id==df_organisationUnitGroups.id,"left").select(df_organisationUnitGroups['name'],'organisationUnits_id')
...
df_Area.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
