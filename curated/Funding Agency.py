...
output_folder_name = 'Funding_Agency'
...
df_dimension = df_dimension.filter(col('name')==lit('Funding Agency'))
...
df_Funding_Agency = (df_dimension.join(df_categoryOptionGroups,df_dimension.items_id==df_categoryOptionGroups.id,"left")      .join(df_categoryOptions,df_categoryOptionGroups.categoryOptions_id==df_categoryOptions.id,'left').select(df_categoryOptionGroups['name'],df_categoryOptions['categoryOptionCombos_id']))

# COMMAND ----------

display(df_Funding_Agency)

# COMMAND ----------

df_Funding_Agency.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
