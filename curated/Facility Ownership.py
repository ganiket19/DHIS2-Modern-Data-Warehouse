...
output_folder_name = 'Facility_Ownership'
...
df_dimension = df_dimension.filter(col('name')==lit('Facility Ownership'))
...
df_Facility_Ownership = df_dimension.join(df_organisationUnitGroups,df_dimension.items_id==df_organisationUnitGroups.id,"left").select(df_organisationUnitGroups['name'],'organisationUnits_id')

# COMMAND ----------

df_Facility_Ownership.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
