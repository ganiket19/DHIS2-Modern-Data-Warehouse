...
output_folder_name = 'Rural_and_Urban'
...
df_dimension = df_dimension.filter(col('name')==lit('Rural and Urban'))
...
df_Rural_and_Urban= df_dimension.join(df_categoryOptions,df_dimension.items_id==df_categoryOptions.id,"left").select(df_categoryOptions['name'],'categoryOptionCombos_id')

# COMMAND ----------

display(df_Rural_and_Urban)

# COMMAND ----------

df_Rural_and_Urban.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
