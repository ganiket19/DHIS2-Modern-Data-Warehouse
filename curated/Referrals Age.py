...
output_folder_name = 'Referrals_Age'
...
df_dimension = df_dimension.filter(col('name')==lit('Referrals Age'))
...
df_Referrals_Age= df_dimension.join(df_categoryOptions,df_dimension.items_id==df_categoryOptions.id,"left").select(df_categoryOptions['name'],'categoryOptionCombos_id')

# COMMAND ----------

display(df_Referrals_Age)

# COMMAND ----------

df_Referrals_Age.write.format("parquet").option("mergeSchema", "true").option("overwriteSchema", "true").mode("Overwrite").save(output_folder_path)
